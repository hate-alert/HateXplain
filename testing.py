...
from Preprocess.dataCollect import get_test_data
...
dict_data_folder={
      '2':{'data_file':'Data/total_annotated_two.pkl','class_label':'Data/classes_two.npy'},
      '3':{'data_file':'Data/total_annotated.pkl','class_label':'Data/classes.npy'}
}
...
## currently only on the current dataset
def softmax(x):
...
def return_params(path_name,att_lambda):
...
def transform_dummy_data(sentences):
    ...
    explanation_list=['{}']*len(sentences)
    old_list=['old']*len(sentences)
    df=pd.DataFrame(list(zip(post_id_list,sentences,pred_list,explanation_list,pred_list,explanation_list,
                             pred_list,explanation_list,old_list,pred_list)),
                         columns=['post_id', 'text', 'pred1', 'explain1','pred2', 'explain2', 'pred3', 'explain3', 'old_vs_new', 'final_annotation',
       ])
    return df
...
def standaloneEval_with_lime(params, model_to_use,test_data=None,topk=2,rational=False):
    ...
    model=modelPred(model_to_use,params)
    ...
    if(rational==True):
        ...
        probab_list=model.return_probab(sentence_list)
        ...
        for index,row in tqdm(test_data.iterrows(),total=len(test_data)):
            ...
            exp = explainer.explain_instance(sentence, model.return_probab, num_features=6, top_labels=3,num_samples=params['num_samples'])
...
if __name__=='__main__': 
    ...
    temp_read = pd.read_pickle('Data/total_annotated.pkl')
    with open('Data/post_id_divisions_modified.json', 'r') as fp:
        post_id_dict=json.load(fp)
    temp_read=temp_read[temp_read['post_id'].isin(post_id_dict['test'])]
    
    
    
    params=return_params(model_dict_params[model_to_use],float(args.extra))
