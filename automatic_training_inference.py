...
def get_gpu():
    ...
    while(1):
        tempID = [] 
        tempID = GPUtil.getAvailable(order = 'memory', limit = 1, maxLoad = 0.5, maxMemory = 0.5, includeNan=False, excludeID=[], excludeUUID=[])
        if len(tempID) > 0:
            print("Found a gpu")
...
if __name__=='__main__':  
    with open('all_params_scrat.json', 'r') as read_file:
        data = json.load(read_file)
    try:
        print(sys.argv[1])
        params=data[int(sys.argv[1])]
    except:
        sys.exit(0)
